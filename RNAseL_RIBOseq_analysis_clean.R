...
all <- read.csv("./data/Ribo_count.csv", row.names = 1) 
all <- round(all)
...
# Load metadata
metadata <- read.delim2("./data/Metadata_footprint_clean.txt", sep = "\t", row.names = 1, header = T)
...
for (variable in c("Sequencing_pool","Genotype","Inducer")){
...
# It was created a new column in metadata (Group_gt_ind) that concatenates the info from Genotype and Inducer columns so coefficients include genotype info.
# It was also added a new column (Read_counts) to tag samples with High or Low sequencing depth so this factor can be controlled for in the design formula.
# The design formula use in DESeq2 is the following:
  
# add new factors (Group_gt_ind)
...
keep <- rowSums(counts(dds.one.RS)) >= 0 # Keep all genes for the analysis
...
# Keep genes with at least N_reads reads total across samples
