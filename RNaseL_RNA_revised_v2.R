...
metadata <- metadata[colnames(all),]  ### NOTE: I would provide a metadata file already filtered for the paper
...
### NOTE: we could remove datasets we are not using in the paper and then we won't need sub-setting
meta_one_cds <- subset(metadata, metadata$Dataset == "one") ### NOTE: Dataset entries are all 'one'
all_one_cds <- all[, rownames(meta_one_cds)] ### NOTE: nothing change
...
# Keep genes with at least N reads total across samples
keep <- rowSums(counts(dds.one.cds)) >= N_reads
...
# Agnes separated 2-5A vs poly I:C 
mean_norm_counts.wt <- apply(counts(dds.one.cds.wt, normalized=TRUE), 1, function(x) mean(x))
mean_norm_counts.ko <- apply(counts(dds.one.cds.ko, normalized=TRUE), 1, function(x) mean(x))

write.table(x = as.data.frame(mean_norm_counts.wt), file = './Mean_norm_counts/mean_norm_counts.wt.cds.txt', sep = "\t",  )
write.table(x = as.data.frame(mean_norm_counts.ko), file = './Mean_norm_counts/mean_norm_counts.ko.cds.txt', sep = "\t" )
...
# Genes significant in polyIC-WT (adj.p <= 0.05) but not in polyIC-KO or 25A-WT (adj.p > 0.25)
...
p5 <- ggplot(res_RNaseA_25A, aes(x=res_RNaseA_25A$log2FoldChange.x, y=res_RNaseA_25A$log2FoldChange.y)) + 
